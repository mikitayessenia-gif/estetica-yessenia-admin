...
// Pagination state
let dailyViewOffset = 0; // 0 = today, 1 = tomorrow, etc.
...
async function loadDailyView() {
  ...
  // Update the date input to reflect current view
  const nowDate = new Date();
  nowDate.setHours(0, 0, 0, 0);
  const targetDate = new Date(nowDate);
  targetDate.setDate(nowDate.getDate() + dailyViewOffset);
  document.getElementById('filterDate').valueAsDate = targetDate;
  ...
  turnos.forEach(t => {
    ...
    const precioFinanzaSection = t.estado === "Reservado" ? `
        <div class="turn-finances-integrated">
          <div class="fin-total-row">
            ...
            <span class="val total-price">$${Number(t.precioTotal).toLocaleString('es-AR')}</span>
          </div>
    ...
    const clientSection = t.clienteNombre
      ? `<div class="turn-client-row">
          ...
          ${t.clienteTelefono ? `<a href="https://wa.me/${t.clienteTelefono}" target="_blank" class="whatsapp-link"><i class="fa-brands fa-whatsapp"></i></a>` : ''}
          ${origenBadge}
        </div>`
    ...
    div.innerHTML = `
      <div class="turn-header">
        ...
        <span class="turn-id" title="ID del turno">${t.id}</span>
        ${estadoBadge}
      </div>
      ${clientSection}
  ...
  const isToday = dailyViewOffset === 0;
...
function getBusinessDaysInMonth(year, month) {
  // Get all business days (Mon-Fri) for a given month
  const days = [];
  ...
  allDaysData.forEach((dayInfo, dayIdx) => {
    ...
    sortedTurnos.forEach(t => {
      ...
      if (t.estado === "Reservado") {
        ...
        html += `<div class="fin-total-row"><span class="lbl">Precio Total</span><span class="val total-price">$${Number(t.precioTotal).toLocaleString('es-AR')}</span></div>`;
...
async function prepareBookForm() {
  ...
  tratamientosDisponibles.forEach(t => {
    ...
    selectT.innerHTML += `<option value="${t.id}" data-precio="${t.precio}" data-filas="${t.duracionFilas}">${t.nombre} - $${t.precio}${duracionLabel}</option>`;
  });
...
async function loadAvailableSlots() {
  ...
  // Reset view mode to "all" when loading slots
  setBookViewMode('all', true);
  ...
  try {
    ...
    filteredDates.forEach(dayInfo => {
      ...
      html += `<div class="date-accordion-header" data-date="${dayInfo.fecha}" style="padding:10px 14px;border-radius:8px;cursor:pointer;display:flex;align-items:center;justify-content:space-between;background:rgba(126,200,200,0.08);border:1px solid rgba(126,200,200,0.2);margin-bottom:2px;transition:all 0.3s;grid-column:1/-1">`;
      html += `<span style="color:#2D8B8B;font-size:0.78rem;font-weight:600">${displayDate}</span>`;
      html += `<span style="color:rgba(45,139,139,0.7);font-size:0.72rem">${cantidadTurnos} turno${cantidadTurnos > 1 ? 's' : ''} <span class="accordion-arrow" style="display:inline-block;transition:transform 0.3s;margin-left:4px">▼</span></span>`;
      html += `</div>`;
      ...
      header.addEventListener('click', function () {
        ...
        grid.querySelectorAll('.date-accordion-header').forEach(h => {
          if (h !== header) {
            ...
            h.style.background = 'rgba(126,200,200,0.08)';
          }
        });
        ...
        if (content) {
          ...
          this.style.background = !isOpen ? 'rgba(126,200,200,0.25)' : 'rgba(126,200,200,0.08)';
        }
      });
    });
...
function setBookViewMode(mode, skipReload) {
  ...
  if (mode === 'all') {
    // Active state for "Ver todos" - aqua/teal
    allBtn.style.background = 'rgba(126,200,200,0.3)';
    allBtn.style.borderColor = 'rgba(126,200,200,0.5)';
    allBtn.style.color = '#2D8B8B';
    allBtn.style.boxShadow = '0 2px 8px rgba(126,200,200,0.3)';
    // Inactive state for "Calendario"
    calBtn.style.background = 'rgba(126,200,200,0.05)';
    calBtn.style.borderColor = 'rgba(126,200,200,0.2)';
    calBtn.style.color = 'rgba(45,139,139,0.4)';
    
    if (miniCal) miniCal.style.display = 'none';
    if (grid) grid.style.display = ''; // restore display
    
    if (!skipReload && document.getElementById('bookTratamiento').value) {
      loadAvailableSlots();
    }
  } else if (mode === 'calendar') {
    // Inactive state for "Ver todos"
    allBtn.style.background = 'rgba(126,200,200,0.05)';
    allBtn.style.borderColor = 'rgba(126,200,200,0.2)';
    allBtn.style.color = 'rgba(45,139,139,0.4)';
    allBtn.style.boxShadow = 'none';
    // Active state for "Calendario" - aqua/teal
    calBtn.style.background = 'rgba(126,200,200,0.3)';
    calBtn.style.borderColor = 'rgba(126,200,200,0.5)';
    calBtn.style.color = '#2D8B8B';
    calBtn.style.boxShadow = '0 2px 8px rgba(126,200,200,0.3)';
...
// ========== DAR TURNO - Mini Calendar ==========
let bookCalendarDate = new Date();

function renderBookCalendar() {
  const calDays = document.getElementById('bookCalendarDays');
  const calDates = document.getElementById('bookCalendarDates');
  const monthYear = document.getElementById('bookCalendarMonthYear');
  
  if (!calDays || !calDates || !monthYear) return;

  const year = bookCalendarDate.getFullYear();
  const month = bookCalendarDate.getMonth();
  
  const meses = ['Enero', 'Febrero', 'Marzo', 'Abril', 'Mayo', 'Junio', 'Julio', 'Agosto', 'Septiembre', 'Octubre', 'Noviembre', 'Diciembre'];
  monthYear.textContent = `${meses[month]} ${year}`;

  // Set grid on the container itself
  calDays.style.display = 'grid';
  calDays.style.gridTemplateColumns = 'repeat(7, 1fr)';
  calDays.style.gap = '4px';
  calDays.style.textAlign = 'center';
  
  // Clear any existing content
  calDays.innerHTML = '';

  // Day headers (Lu, Ma, Mi...)
  const diasCab = ['Lu', 'Ma', 'Mi', 'Ju', 'Vi', 'Sá', 'Do'];
  diasCab.forEach(dia => {
    calDays.appendChild(Object.assign(document.createElement('div'), {
      textContent: dia,
      style: 'font-size:0.7rem;font-weight:600;color:rgba(0,0,0,0.4);padding:4px'
    }));
  });
  
  // First day of month
  const firstDay = new Date(year, month, 1).getDay();
  const adjustment = firstDay === 0 ? 6 : firstDay - 1; // Convert to Monday-start
  
  // Get available slot dates for this month
  const hoy = new Date();
  hoy.setHours(0, 0, 0, 0);
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    const isToday = dateObj.toDateString() === hoy.toDateString();
    
    // Check if this date has available slots
    const hasSlots = window._bookAvailableDates && window._bookAvailableDates.includes(dateStr);
    
    let bgStyle = '';
    let cursor = 'default';
    
    if (isToday) {
      bgStyle = 'background:rgba(126,200,200,0.35);font-weight:700;color:#2D8B8B;';
    } else if (hasSlots && !isPast) {
      bgStyle = 'background:rgba(126,200,200,0.15);color:#2D8B8B;font-weight:500;cursor:pointer;';
      cursor = 'pointer';
    } else if (isPast) {
      bgStyle = 'opacity:0.3;';
    }
    ...
    const cell = Object.assign(document.createElement('div'), {
      ...
      style: `padding:6px 4px;border-radius:6px;font-size:0.75rem;${bgStyle}`,
      'data-date': dateStr
    });
    
    if (hasSlots && !isPast) {
      cell.addEventListener('click', () => selectBookCalendarDate(dateStr));
    }
    
    calDays.appendChild(cell);
  }

  // Render list of dates with slots below calendar
  let datesHtml = '';
  if (window._bookAvailableDates && window._bookAvailableDates.length > 0) {
    const sortedDates = [...window._bookAvailableDates].sort((a, b) => {
      const partsA = a.split('/');
      const partsB = b.split('/');
    ...
    sortedDates.forEach(dateStr => {
      const fechaObj = parseDisplayDate(dateStr);
      const diaNombre = fechaObj ? getDiaSemana(fechaObj) : '';
      const displayDate = diaNombre ? `${dateStr} - ${diaNombre}` : dateStr;
      
      datesHtml += `<div onclick="selectBookCalendarDate('${dateStr}')" style="padding:8px 12px;border-radius:8px;cursor:pointer;background:rgba(126,200,200,0.1);border:1px solid rgba(126,200,200,0.25);font-size:0.8rem;color:#2D8B8B;font-weight:500;transition:all 0.2s" onmouseover="this.style.background='rgba(126,200,200,0.2)'" onmouseout="this.style.background='rgba(126,200,200,0.1)'">${displayDate}</div>`;
    });
  }
  
  if (!datesHtml) {
    datesHtml = '<p style="text-align:center;color:var(--text-light);font-size:0.8rem;padding:20px 0">No hay turnos disponibles en este período</p>';
  }
  
  calDates.innerHTML = datesHtml;
}

function changeBookCalendarMonth(delta) {
  bookCalendarDate.setMonth(bookCalendarDate.getMonth() + delta);
  renderBookCalendar();
}

function selectBookCalendarDate(dateStr) {
  // Find the date in filteredDates and expand its accordion
  const grid = document.getElementById('bookSlotsGrid');
  if (!grid) return;
  
  const header = grid.querySelector(`.date-accordion-header[data-date="${dateStr}"]`);
  if (header) {
    header.click(); // Expand this date's accordion
...
document.getElementById('btnConfirmBook').addEventListener('click', async () => {
  ...
  const payload = {
    ...
    precioTotal: opt ? opt.dataset.precio : 0,
