...
async function loadDailyView() {
  ...
  document.getElementById('statsTotal').textContent = 'Total: $' + Number(senaTotal + faltanteTotal).toLocaleString('es-AR');
...
if (btnListMode) {
  btnListMode.addEventListener('click', () => {
    if (listModeActive) {
      ...
      document.getElementById('btnToday').classList.remove('active-today');
      document.getElementById('agendaSearchBar').style.display = 'none';
      const si = document.getElementById('agendaSearchInput');
      if (si) si.value = '';
      hidePagination();
      loadDailyView();
    } else {
      ...
      document.getElementById('btnToday').classList.remove('active-today');
      document.getElementById('agendaSearchBar').style.display = 'block';
...
if (agendaSearchInput) {
  agendaSearchInput.addEventListener('input', () => {
    const val = agendaSearchInput.value.trim();
    agendaSearchQuery = val;
    if (btnClearSearch) btnClearSearch.classList.toggle('visible', val.length > 0);
    filterAgendaList();
  });
}
...
async function loadAgendaList() {
  ...
  // Calculate summary stats (accumulated across all days)
  let totalReservados = 0, totalDisponibles = 0, totalBloqueados = 0;
  let totalSena = 0, totalFaltante = 0;
  allDaysData.forEach(d => {
    d.turnos.forEach(t => {
      if (t.estado === 'Reservado') {
        totalReservados++;
        totalSena += Number(t.montoAbonado) || 0;
        totalFaltante += Number(t.montoFaltante) || 0;
      }
  ...
  // Update top stat cards with accumulated values
  document.getElementById('statReservados').textContent = totalReservados;
  document.getElementById('statSena').textContent = '$' + Number(totalSena).toLocaleString('es-AR');
  document.getElementById('statFaltante').textContent = '$' + Number(totalFaltante).toLocaleString('es-AR');
  document.getElementById('statsTotal').textContent = 'Total: $' + Number(totalSena + totalFaltante).toLocaleString('es-AR');
  ...
  allDaysData.forEach((dayInfo, dayIdx) => {
    ...
    sortedTurnos.forEach(t => {
      ...
      const turnIdDisplay = t.id || '';
      html += `<div class="list-turn-row" data-turn-id="${turnIdDisplay}" onclick="toggleTurnDetails('${t.id}')">`;
      html += `<span class="list-turn-time">${t.horaInicio || '--:--'} - ${t.horaFin || '--:--'}</span>`;
      html += `<span class="list-turn-id-badge" title="ID del turno en Google Sheets">${turnIdDisplay}</span>`;
      ...
      if (t.clienteNombre) {
        ...
        if (t.clienteTelefono) {
          html += `<p style="margin-bottom:4px;font-size:0.82rem;color:#999;font-style:italic"><a href="https://wa.me/${waPhone(t.clienteTelefono)}" target="_blank" style="color:#25D366;text-decoration:none;font-size:0.9rem"><i class="fa-brands fa-whatsapp"></i></a> ${t.clienteTelefono}</p>`;
        }
      }
...
function toggleTurnDetails(turnId) {
  ...
  if (details) {
    ...
    // Recalculate parent day body max-height so expanded detail doesn't get clipped
    const body = details?.closest('.list-day-body');

    if (details.classList.contains('open')) {
      // First remove inline max-height so CSS .open applies, then measure
      details.style.maxHeight = 'none';
      void details.offsetHeight; // force reflow
      const h = details.scrollHeight;
      details.style.maxHeight = h + 'px';

      // Remove body constraint, measure, set
      if (body) {
      ...
      // Collapsing - recalc body after transition
      if (body) {
        setTimeout(() => {
          void body.offsetHeight;
          body.style.maxHeight = body.scrollHeight + 'px';
...
function filterAgendaList() {
  ...
  const dayGroups = document.querySelectorAll('.list-day-group');

  if (!query) {
    // No filter - show everything
    dayGroups.forEach(g => g.style.display = '');
    document.querySelectorAll('.list-turn-row').forEach(r => r.style.display = '');
    document.querySelectorAll('.list-turn-details').forEach(d => {
      d.classList.remove('open');
      d.style.maxHeight = '0';
    });
    document.querySelectorAll('.list-turn-row.expanded').forEach(r => r.classList.remove('expanded'));
    return;
  }

  dayGroups.forEach(group => {
    const rows = group.querySelectorAll('.list-turn-row');
    const body = group.querySelector('.list-day-body');
    let anyVisible = false;

    // Temporarily remove body constraint so details can measure full height
    if (body) {
      body.style.maxHeight = 'none';
      body.style.overflow = 'visible';
    }
    ...
    rows.forEach(row => {
      const turnId = (row.getAttribute('data-turn-id') || '').toLowerCase();
      const rowText = row.textContent.toLowerCase();
      const detailsEl = document.getElementById('details-' + row.getAttribute('data-turn-id'));
      const detailsText = detailsEl ? detailsEl.textContent.toLowerCase() : '';

      const searchPattern = query.startsWith('t-') ? query : 't-' + query;
      const matches = turnId.includes(searchPattern) || rowText.includes(query) || detailsText.includes(query);

      row.style.display = matches ? '' : 'none';
      if (matches) {
        anyVisible = true;
        // Auto-expand matching turn details
    ...
    // Auto-expand matching day groups
    if (anyVisible) {
